...
def list_products(
    ...
    q = db.query(Product).options(joinedload(Product.category)).filter(Product.deleted_at.is_(None))
    if search:
    ...
    if supplier_id:
        ...
        # Always filter by linked_ids; in_([]) returns no rows, which is correct
        # when a supplier has no linked products (user must add them in Master Data)
        q = q.filter(Product.product_id.in_(linked_ids))
    if in_stock_only:
        # Only products that have total inventory quantity > 0
        in_stock_subq = (
...
def create_product(data: ProductCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(product)
    return db.query(Product).options(joinedload(Product.category)).filter(Product.product_id == product.product_id).first()


@router.get("/{product_id}", response_model=ProductResponse)
def get_product(product_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    product = db.query(Product).options(joinedload(Product.category)).filter(Product.product_id == product_id, Product.deleted_at.is_(None)).first()
    if not product:
...
def update_product(product_id: int, data: ProductUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.commit()
    return db.query(Product).options(joinedload(Product.category)).filter(Product.product_id == product_id).first()
